function RouterRequest() {
	...
	function authorize_(directUrl, appId) {
		if (authorizeError_) {
			return;
		}
	...
	function autoAuthorize_(directUrl, appId) {
		var authorizeUrl = authorizeUrl_;
		authorizeUrl += "?client_id=" + appId;
		authorizeUrl += "&redirect_uri=" + encodeURIComponent(directUrl);
		authorizeUrl += "&response_type=token";
	...
	function request_(params) {
		...
		if (!params.data.token || typeof (params.data.scope) != "string") {
			if (accessToken_) {
				...
				autoAuthorize_(url, params.data.appId);
				return;
			}
		}
		params_stack_ = null;
